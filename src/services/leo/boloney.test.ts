...
describe("Boloney service", () => {
  ...
  it("successfully creates a match", async () => {
    const { owner, matchId, settings, powerUps } = body.createMatch;

    const parsedPowerUps = powerUpProbabilitiesSchema.parse(powerUps);

    const res = await boloney.createMatch(owner, matchId, settings, parsedPowerUps);
  ...
  it("successfully creates a match summary", async () => {
    const { owner, matchId, ranking } = body.createMatchSummary;

    const res = await boloney.createMatchSummary(owner, matchId, ranking);
